#include <stdio.h>
#define N 1     //magaza sayisi kendimiz değiştiriyoruz keyfe göre oynama yapabilirsin
#define M 2     //ürün sayisi  den den den den den den
struct urun
{
    char urun_kodu[20];
    float fiyat;
...
void isciyi_olustur(struct isci *em)
{
    ...
    scanf("%s",&em->isim);
    printf("Iscinin satis sayisi giriniz:\n ");
    scanf("%d",&em->satissayisi);
}
...
void ayinelemani(struct magaza m)
{
    struct isci iyi;
    if (m.e1.satissayisi >iyi.satissayisi)
        iyi = m.e1;
    if (m.e2.satissayisi >iyi.satissayisi)
        iyi = m.e2;
    if (m.e3.satissayisi >iyi.satissayisi)
        iyi = m.e3;
    printf("Ayin elemani:\n");
    isciyi_bas(iyi);
}
